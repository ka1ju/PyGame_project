...
def sde(user, t_choose, t_field, flag):
...
class level():
    ...
    def render(self, pos):
        if 225 <= pos[0] <= 252 and 704 >= pos[1] >= 675:  # up
            posxy = (75, 225)
            chxy = (234, 677)
            upp = sde(self.user, "textures/mini_choose.jpg", "textures/frond_field.jpg", "up")
            choose, flag, field, user = upp[0], upp[1], upp[2], upp[3]
        elif 226 <= pos[0] <= 252 and 705 <= pos[1] <= 763:  # back
            chxy = (234, 706)
            posxy = (75, 15)
            upp = sde(self.user, "textures/big_choose.jpg", "textures/first_side_field.jpg", "back")
            choose, flag, field, user = upp[0], upp[1], upp[2], upp[3]
        elif 171 <= pos[0] <= 197 and 704 <= pos[1] <= 763:  # front
            chxy = (179, 706)
            posxy = (75, 15)
            upp = sde(self.user, "textures/big_choose.jpg", "textures/first_side_field.jpg", "front")
            choose, flag, field, user = upp[0], upp[1], upp[2], upp[3]
            ...
            chxy = (234, 765)
            posxy = (75, 225)
            upp = sde(self.user, "textures/mini_choose.jpg", "textures/back_field.jpg", "down")
            choose, flag, field, user = upp[0], upp[1], upp[2], upp[3]
        return choose, chxy, field, flag, posxy, self.user
    ...
    def step(self, u_pos, user_coords):
        ...
        if flag == u_pos[0]:
            ...
            if posxy[0] < pos[0] < posxy[0] + 340 and (posxy[1] < pos[1] < posxy[1] + 350 or (
                    posxy[1] < pos[1] < posxy[1] + 650 and side != "up" and side != "down")):
                if u_pos[1][0] - 58 <= pos[0] <= u_pos[1][0] + 90 and u_pos[1][1] - 50 <= pos[1] <= u_pos[1][1] + 100:
                    ...
                    usr.rect = pygame.Rect((u_pos[1][0] + 2, u_pos[1][1] + 2, 48, 48))
                    if pygame.sprite.spritecollideany(usr, all_sprites) is None:
                        return user_coords, u_pos
        return f_user_coords, f_u_pos

    def turnit(self, u_pos):
        if up_flag and 227 <= pos[0] <= 275 and 3 <= pos[1] <= 15:
            if u_pos[0] == "front":
                side = "up"
                user_coords = (225, 525)
                upp = sde(self.user, "textures/mini_choose.jpg", "textures/frond_field.jpg", "up")
            ...
            chxy = (234, 677)
            choose, flag, field, user = upp[0], upp[1], upp[2], upp[3]
        elif down_flag and (227 <= pos[0] <= 275 and 575 <= pos[1] <= 587) or (
                319 <= pos[0] <= 365 and 664 <= pos[1] <= 675):
            if u_pos[0] == "up":
                side = "front"
                user_coords = (225, 15)
                chxy = (179, 706)
            ...
            u_pos = [side, user_coords]
            choose, flag, field, user = upp[0], upp[1], upp[2], upp[3]
        elif right_flag and 418 <= pos[0] <= 427 and (
                376 <= pos[1] <= 421 or 117 <= pos[1] <= 163 or 67 <= pos[1] <= 113):
            if u_pos[0] == "up":
                side = "right"
                user_coords = (225, 15)
                chxy = (261, 706)
            ...
            choose, flag, field, user = upp[0], upp[1], upp[2], upp[3]
                ...
                56 <= pos[0] <= 66 and (115 <= pos[1] <= 161 or 65 <= pos[1] <= 111 or 275 <= pos[1] <= 321)):
            if u_pos[0] == "right":
                side = "back"
                user_coords = (375, 115)
                chxy = (234, 706)
            ...
            u_pos = [side, user_coords]
            choose, flag, field, user = upp[0], upp[1], upp[2], upp[3]
        return choose, flag, field, u_pos, chxy, posxy, side, user_coords
...
def check_turn():
    ...
    if u_pos[1] == (225, 15) and flag == u_pos[0]:
        scr.blit(goup, (227, 3))
        up_flag = True
    elif u_pos[1] == (225, 525) and flag == u_pos[0]:
        scr.blit(godown, (227, 575))
        down_flag = True
    elif u_pos[1] == (375, 375) and flag == u_pos[0]:
        scr.blit(goright, (380, 349))
        right_flag = True
    elif u_pos[1] == (75, 115) and flag == u_pos[0]:
        scr.blit(goleft, (24, 90))
        left_flag = True
    elif u_pos[1] == (375, 115) and flag == u_pos[0]:
        scr.blit(goright, (380, 91))
        right_flag = True
    elif u_pos[1] == (75, 65) and flag == u_pos[0]:
        scr.blit(goleft, (24, 40))
        left_flag = True
    elif u_pos[1] == (375, 65) and flag == u_pos[0]:
        scr.blit(goright, (380, 41))
        right_flag = True
    elif u_pos[1] == (325, 615) and flag == u_pos[0]:
        scr.blit(godown, (327, 665))
        down_flag = True
    elif u_pos[1] == (75, 275) and flag == u_pos[0]:
        scr.blit(goleft, (24, 250))
        left_flag = True
    return up_flag, left_flag, right_flag, down_flag
...
while running:
    for event in pygame.event.get():
        ...
        ct = check_turn()
        up_flag, left_flag, right_flag, down_flag = ct[0], ct[1], ct[2], ct[3]
        if event.type == pygame.QUIT:
            running = False
        if event.type == pygame.MOUSEBUTTONUP:
            pos = pygame.mouse.get_pos()
            print(user_coords)
            if (pos[1] > posxy[1] + 350 and flag == "up" or flag == "down") or pos[1] > posxy[1] + 650:
                try:
                    first_is = True
                    all_sprites.remove(*[el for el in all_sprites])
                    rend = level.render(level(), pos)
                    choose, chxy, field, flag, posxy, user = rend[0], rend[1], rend[2], rend[3], rend[4], rend[5]
                except UnboundLocalError:
                    pass
            try:
                stp = level.step(level(), u_pos, user_coords)
                user_coords, u_pos = stp[0], stp[1]
            except UnboundLocalError:
                pass
            if up_flag or down_flag or right_flag or left_flag:
                try:
                    turn = level.turnit(level(), u_pos)
                    choose, flag, field, u_pos, chxy, posxy, side, user_coords = turn[0], turn[1], turn[2], turn[3], \
